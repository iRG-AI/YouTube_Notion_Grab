...
function geminiCall(prompt, apiKey) {
...
- **제목 우선 신호**: 제목에 도구/주제명이 명시되거나 대상 독자가 명시되면 해당 토픽 반드시 포함
  - "바이브 코딩으로 미니앱 개발" → "AI 바이브코딩" ✅ (행위)
  - 제목에 "바이브코더" 포함 → "AI 바이브코딩" ✅ (대상 독자/행위자 모두 커버)
  - AI 코딩/AI 개발 관련 내용 (코드 품질, DDD, TDD, 기술부채, AI로 코드 작성 실천론) → "AI 바이브코딩" ✅
  - "NotebookLM으로 슬라이드 만들기" → "AI 노트북 LM" ✅
  - AI RAG 핵심 원리(칭킹, 하이브리드 검색, 벡터 DB), 지식 관리/베이스 시스템 구축, Obsidian 연동 개인 지식 위키 → "AI LLM Wiki" ✅
  - 제목이 명확하면 요약 본문이 부족해도 토픽 인정
- **채널 신호**: 채널 이름이 특정 도구/주제를 명시하고, 영상 내용이 그 맥락에 해당하면 해당 토픽 포함
  - 채널명에 "바이브코딩" 포함 + 코딩·개발 관련 내용 → "AI 바이브코딩" ✅
  - 채널명에 특정 AI 도구명 포함 + 해당 도구 내용 → 그 도구 토픽 포함 ✅
...
- "AI 자동화" → 영상이 *자동화 파이프라인/봇 구축*을 핵심으로 다룰 때만
  - ❌ 자동화 효과 언급만 → 제외
  - ✅ n8n/Make/Zapier/스크립트 자동화, AI 에이전트 기반 영상/콘텐츠 대량 생산 파이프라인 구축 및 워크플로우 → 포함
...
async function classifyTopics({ summary, title, channel }, availableTopics, opts = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      const raw = await geminiCall(prompt, opts.geminiApiKey);
